...
```sh
.agents/skills/add-package/pkg-lookup all <colloquial-name>
```
...
search Flathub with the partial name (`.agents/skills/add-package/pkg-lookup flatpak <name>`) and
GitHub releases (`.agents/skills/add-package/pkg-lookup appimage <owner/repo>`) if the user mentions a
GitHub source.
...
4. **GUI app, macOS**: if `.agents/skills/add-package/pkg-lookup cask <name>` hits, set `brew_cask: true`.
...
- **`fd` vs `fd-find`**: classic case where the apt and dnf packages live under
  ...
  `.agents/skills/add-package/pkg-lookup apt <name>` and `.agents/skills/add-package/pkg-lookup dnf <name>` — do not assume.
...
- **AppImage rate-limits**: GitHub's unauthenticated API allows 60 requests/hr.
  If `.agents/skills/add-package/pkg-lookup appimage` returns a network error, try again later — do not
  guess the repo.
